...
/**
 * The Fabric carrier for one engine enum axis (sub-07 Stage B): a vanilla
 * {@link net.minecraft.state.property.Property} whose values are the descriptor's
 * declared enum constants, in declaration order.
 *
 * <p><b>Why not vanilla's {@code EnumProperty}:</b> it is bound to
 * {@code T extends Enum<T> & StringIdentifiable}, and an engine-owned enum can
 * never satisfy that — {@code StringIdentifiable} is a game interface, and the
 * enum type a descriptor names is shared by every cell (a headless runtime has no
 * Minecraft class to implement it with). This carrier keeps the vanilla contract
 * that matters (a native property with the declared value set, in the declared
 * order, carrying the property's own value codec) and supplies the naming the
 * native state grammar demands.
 *
 * <p><b>Naming, and why it is not the engine's:</b> a native state manager only
 * accepts lowercase value names ({@code [a-z0-9_]+}, validated by
 * {@code StateManager.Builder#add} through {@link #name(Object)}), while an
 * engine enum's names are Java constant names ({@code IDLE}), and the engine's
 * own state string pins them ({@code mode=IDLE}). The native name is therefore
 * the lowercased constant name — the same convention {@code StringIdentifiable}
 * users follow — and the mapping back to the engine value goes through the
 * declared constant itself, never through a name. The engine's state identity
 * stays exactly what the descriptor declared, value order included (the engine's
 * flattening order). A constant whose name cannot be spoken natively is an
 * authoring error and is rejected here, naming the property, the constant and the
 * offending name.
 *
 ...
 * default (vanilla builds the default state from each property's first value),
 * which is the engine's declared default too. Immutable.
 *
 * @param <E> the engine's enum type, as declared by the descriptor's property
 */
public final class EngineEnumProperty<E extends Enum<E>> extends net.minecraft.state.property.Property<E> {
     ...
     * declared order <em>is</em> the engine's flattening order and it is the order
     * the native state table is generated in (vanilla's {@code EnumProperty} hands
     * back an unordered set; a driver may not).
     */
    @Override
    ...
    @Override
    public Optional<E> parse(String nativeName) {
