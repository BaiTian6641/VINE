...
- [ ] **Do:** `requires` predicates, `redirect`, remaining vanilla-mirror argument types, `suggests`, JSON descriptor path (sub-02 machinery), merge/conflict policy.
...
- [ ] **Do:** `VINE_ID`, `VOXEL_PATH`, `PLAYER_IN_SESSION` parsers + suggestion defaults; synced-tree mapping to nearest vanilla types; `VINE_ID` suggests from engine registries (sub-02).
- **Acceptance:** TCK argument scenario — all three types parse/suggest server-side on both loaders; client-smoke: unmodded vanilla client sees the command tree, tab-completes, and never errors on unknown argument types.
