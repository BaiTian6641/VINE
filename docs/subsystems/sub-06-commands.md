...
- [x] **Do:** `Node` permission resolution: Fabric → fabric-permissions-api if present else fallback level; NeoForge → loader permission handler else fallback; all server-side.
- **Landed:** `VinePermission.node(name, fallbackLevel)`; precedence lives in `PermissionGates.decide` (vine-api, pure Java): an engine-registered `VinePermissionBridge` wins, then the cell's native provider when it has an opinion, then the descriptor's fallback op level. A bridge is installed through `VineCommands.registerPermissionBridge` (not frozen-gated — a plugin may arrive at runtime; `null` clears). Fabric's cell probes fabric-permissions-api reflectively once at boot (`Permissions.check(source, node, fallback)`, never a hard dependency) and reports "no opinion" when the provider agrees with the fallback, so op-level semantics stay the engine's; NeoForge's cell reports no provider *by design* — its 1.21 API replaced string nodes with typed `PermissionNode`s registered at startup (`PermissionAPI.getRegisteredNodes()`, handler id via `getActivePermissionHandler()`), which loader-free consumers cannot mint, and pretending to check an unregistered node would silently deny every gate. Each dispatcher build logs the active path (`[VINE] commands: node permissions via …`).
- **Acceptance met:** TCK `command_permission` — both cells install a provider and observe grant + denial (`calls=2 last=vine_test.perm.deny verdict=false`: a denied source is filtered out of the tree, so the denial is observed through the provider's own record), then clear it and observe the unclaimed node executing through the fallback level; the precedence matrix (bridge > native > fallback, including the fallback *denial* branch that no headless source can reach — the console is always op-4) is exercised in-process by the `perm tck_policy` probe.
...
- **Landed (reload half):** every native dispatcher build re-reads the JSON layer before it pulls the snapshot — one call site in the driver-side attach walk covers boot *and* each `/reload`, so no ordering guess exists between the loader's reload event and command registration (Fabric's `CommandRegistrationCallback` fires during resource loading; NeoForge's `RegisterCommandsEvent` fires on every reload — both attach through the same path). Discovery now also reads each consumer's dev resource output: Loom and ModDevGradle serve mod resources from remapped or relocated jars, so the module's `build/resources/main` was invisible to the classloader walk, and an edited descriptor would have replayed the jar's contents; the URL-scheme and sibling derivation lives in `vine-api` `DevPaths` (file/jar/union schemes) and is shared with the testmod's fixture writer. Boot keeps its own initializer-time load so a malformed consumer file fails at startup rather than at the first reload.
- **Acceptance met (reload half):** TCK `command_reload` — the scenario writes a brand-new descriptor into the mod's dev resource output, runs `/reload`, and executes the command it declares (`reload probe: descriptor live`) with the dispatcher-build log confirming the recompile; it then removes the file and reloads again. Green on both 1.21.1 cells.
- **Remaining (client half):** `vine-client-api`/client drivers need a client runner, which no stage provides yet (sub-21 automates server-side boots only); a headless dedicated server cannot host a client command or prove its invisibility to the server. Recorded as a checklist item, not claimed here.
...
- **Stage A driver attach (resolved 2026-09-24, was: in flight):** engine side landed — vine-api `dev.vineengine.vine.command` (descriptor DSL subset, `VineCommands`/`CommandBackend` via the EngineAccess seam), vine-core `dev.vineengine.vine.internal.command` (`CommandService` freeze-aware store + `CommandCompiler` + `CommandBridge` driver seam: `commandsForNativePass()` pull + `execute(...)` with error isolation). The 1.21.1 drivers' native registration pass pulls the snapshot and attaches nodes on every dispatcher build (`RegisterCommandsEvent` / `CommandRegistrationCallback`, eager-but-dormant listener), so engine descriptors execute through each cell's own dispatcher — `command_echo`, `command_tree` and `command_permission` prove it on both 1.21.1 cells; `CommandQueue` stays sub-18's bare-literal hook mechanism and does NOT carry engine descriptors (no children/permission/feedback channel, and vine-core cannot push into a driver-side queue).
